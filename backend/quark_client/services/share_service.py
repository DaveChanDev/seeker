...
class ShareService:
    ...
    def get_share_info(self, share_id: str, token: str, pdir_fid: str = "0", page: int = 1, size: int = 500, fetch_all: bool = True) -> Dict[str, Any]:
        """
        获取分享详细信息
        ...
        if fetch_all and isinstance(response, dict) and response.get('code') == 0:
            ...
            while len(items) < total:
                ...
                next_params['_page'] = cur_page
                next_resp = self.client.get(
                    'share/sharepage/detail',
                    params=next_params,
                    base_url=Config.SHARE_BASE_URL
                )
                if isinstance(next_resp, dict) and next_resp.get('code') == 0:
                    next_list = next_resp.get('data', {}).get('list', [])
                    if not next_list:
                        break
                    items.extend(next_list)
                else:
                    break
            data['list'] = items
